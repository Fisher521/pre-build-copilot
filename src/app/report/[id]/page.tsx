      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-gray-50 to-white">
        <StepCard maxWidth="md">
          <div className="text-center py-8">
            ...
            <button onClick={handleRestart} className="px-6 py-2.5 bg-primary-500 text-white rounded-xl hover:bg-primary-600 transition-colors">
              重新开始
            </button>
          </div>
        </StepCard>
  ...
  return (
    <div className="min-h-screen py-12 px-6 bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Score Card */}
        <div className={cn('rounded-2xl border-2 p-8 mb-6 text-center', getScoreBg(report.score))}>
          <div className={cn('text-6xl font-bold mb-2', getScoreColor(report.score))}>{report.score}</div>
          <div className="text-gray-500 text-sm mb-3">可行性评分 / 100</div>
          <div className={cn('inline-block px-4 py-1.5 rounded-full text-sm font-medium', report.score >= 60 ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700')}>
            {report.verdict}
          </div>
        </div>
        ...
        <StepCard className="mb-8">
          ...
          <ol className="space-y-3">
            {report.nextSteps.map((item, i) => (
              <li key={i} className="flex items-start gap-3 text-gray-700">
                <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary-100 text-primary-600 flex items-center justify-center text-sm font-medium">{i + 1}</span>
                <span>{item}</span>
              </li>
            ))}
          </ol>
        ...
        <div className="flex items-center justify-center gap-4">
          <button onClick={handleRestart} className="px-6 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-100 transition-colors">
            重新评估
          </button>
        </div>
      </div>
